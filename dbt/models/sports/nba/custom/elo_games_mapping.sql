...
elo_game_team_ids as (
    select
        ...
        upper(trim(cast(team_id as string))) as team_id,
        upper(trim(cast(opp_id as string))) as opp_id,
        safe_cast(kaggle_team_id as int64) as kaggle_team_id,
        safe_cast(kaggle_opp_team_id as int64) as kaggle_opp_team_id,
        safe_cast(kaggle_home_team_id as int64) as kaggle_home_team_id
...
final as (
    select
        elo.*,
        coalesce(
            game_id_match.kaggle_team_id,
            date_team_match.kaggle_team_id
        ) as kaggle_team_id,
        coalesce(
            game_id_match.kaggle_opp_team_id,
            date_team_match.kaggle_opp_team_id
        ) as kaggle_opp_team_id,
        coalesce(
            game_id_match.kaggle_home_team_id,
            date_team_match.kaggle_home_team_id
        ) as kaggle_home_team_id

    from elo

    left join elo_game_team_ids as game_id_match
        on elo.game_id = game_id_match.game_id

    left join elo_game_team_ids as date_team_match
        on game_id_match.game_id is null
        and elo.date_game = date_team_match.date_game
        and upper(trim(elo.team_id)) = date_team_match.team_id
        and upper(trim(elo.opp_id)) = date_team_match.opp_id
),
